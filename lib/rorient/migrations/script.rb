    ...
    class Script
      ...
      def on_success
        ...
        when "rollback"
          ...
          migration_record = @database.driver.query.execute(query_text: URI.encode("SELECT FROM #{@database.history} WHERE type = 'migration' AND time = " + @datetime + " ORDER BY executed DESC LIMIT 1"))[:result].first
